...
func (r *Runtime) functionCtor(args []Value, proto *Object, async, generator bool) *Object {
	...
	if len(args) > 1 {
		ar := args[:len(args)-1]
		for i, arg := range ar {
			sb.WriteString(ValueToJsString(arg))
			if i < len(ar)-1 {
				sb.WriteRune(',')
			}
		}
	...
	if len(args) > 0 {
		sb.WriteString(ValueToJsString(args[len(args)-1]))
	}
	sb.WriteString(asciiString("\n})"))
...
lenNotInt:
	...
	v := &Object{runtime: r}
	ff := r.newNativeFuncAndConstruct(v, r.boundCallable(fcall, call.Arguments), r.boundConstruct(v, construct, call.Arguments), nil, ValueToUnistring(nameStr), l)
	bf := &boundFuncObject{
		nativeFuncObject: *ff,
		wrapped:          obj,
	}
