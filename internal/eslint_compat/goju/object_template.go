...
func (f *templatedFuncObject) source() String {
	return newStringValue(fmt.Sprintf("function %s() { [native code] }", ValueToJsString(nilSafe(f.getStr("name", nil)))))
}
...
func (a *templatedArrayObject) _setOwnIdx(idx uint32) {
	...
	l := uint32(ValueToInteger(lenProp.value))
	if idx >= l {
...
func (a *templatedArrayObject) setLength(l uint32, throw bool) bool {
	lenProp := a.getLenProp()
	oldLen := uint32(ValueToInteger(lenProp.value))
	if l == oldLen {
		return true
	}
	if !lenProp.writable {
...
func (a *templatedArrayObject) setOwnIdx(p valueInt, v Value, throw bool) bool {
	if !a.templatedObject.setOwnStr(ValueToUnistring(p), v, throw) {
		return false
	}
...
func (a *templatedArrayObject) defineOwnPropertyIdx(p valueInt, desc PropertyDescriptor, throw bool) bool {
	if !a.templatedObject.defineOwnPropertyStr(ValueToUnistring(p), desc, throw) {
		return false
	}
	if idx := toIdx(p); idx != math.MaxUint32 {
		a._setOwnIdx(idx)
