...
func TestSetFunc(t *testing.T) {
	...
	err := r.Set("sum", func(call FunctionCall) Value {
		return r.ToValue(ValueToInteger(call.Argument(0)) + ValueToInteger(call.Argument(1)))
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecursiveRun(t *testing.T) {
	...
	if !ValueSameAs(res, valueInt(401)) {
		t.Fatal(res)
	}
}
...
func TestObjectGetSet(t *testing.T) {
	...
	if o1, ok := v.(*Object); ok {
		if v1 := o1.Get("test"); ValueExport(v1) != int64(43) {
			t.Fatalf("Unexpected test value: %v (%T)", v1, ValueExport(v1))
		}
	}
}
...
func TestThrowFromNativeFunc(t *testing.T) {
	...
	if !ValueEquals(v, asciiString("testError")) {
...
func TestSetFuncVariadic(t *testing.T) {
	...
	vm.Set("f", func(s string, g ...Value) {
		something := ValueToInteger(ValueToObject(vm, g[0]).Get(s))
		if something != 5 {
			t.Fatal()
		}
	})
...
func TestRuntime_ExportToCallable(t *testing.T) {
	...
	if err != nil {
		t.Fatal(err)
	} else if !ValueStrictEquals(res, vm.ToValue(42)) {
...
func TestRuntime_ExportToObject(t *testing.T) {
	...
	if v := o.Get("test"); !ValueStrictEquals(v, vm.ToValue(42)) {
...
func TestJsonEncodable(t *testing.T) {
	...
	if !ValueStrictEquals(ret, vm.ToValue("\"Test\"")) {
...
func TestNilCallArg(t *testing.T) {
	...
	if f, ok := AssertFunction(vm.Get("f")); ok {
		...
		if !ValueStrictEquals(v, valueTrue) {
			t.Fatalf("Unexpected result: %v", v)
		}
	}
}
...
func TestReflectCallExtraArgs(t *testing.T) {
	...
	if !ValueStrictEquals(res, intToValue(42)) {
...
func (t *testNativeConstructHelper) calc(call FunctionCall) Value {
	return t.rt.ToValue(t.base + ValueToInteger(call.Argument(0)))
}
...
func TestNativeConstruct(t *testing.T) {
	...
	rt.Set("F", func(call ConstructorCall) *Object { // constructor signature (as opposed to 'func(FunctionCall) Value')
		h := &testNativeConstructHelper{
			rt:   rt,
			base: ValueToInteger(call.Argument(0)),
		}
	...
	if !ValueStrictEquals(res, valueTrue) {
		t.Fatalf("Unexpected result: %v", res)
	}
...
func TestRuntimeNew(t *testing.T) {
	...
	if n, ok := ValueExport(v).(int64); ok {
		if n != 12345 {
			t.Fatalf("n: %v", n)
		}
	} else {
...
func TestToValueNilValue(t *testing.T) {
	...
	if !ValueSameAs(asciiString("null"), ret) {
		t.Fatalf("ret: %v", ret)
	}
}
...
func TestDateConversion(t *testing.T) {
	now := time.Now()
	vm := New()
	val, err := vm.New(ValueToObject(vm, vm.Get("Date")), vm.ToValue(now.UnixNano()/1e6))
	if err != nil {
		t.Fatal(err)
	}
	vm.Set("d", val)
	...
	if exp := ValueExport(res); exp != now.UnixNano()/1e6 {
...
func TestNativeCtorNonNewCall(t *testing.T) {
	...
	vm.Set(`Animal`, func(call ConstructorCall) *Object {
		obj := call.This
		obj.Set(`name`, ValueToString(call.Argument(0)))
...
func ExampleNewSymbol() {
	sym1 := NewSymbol("66")
	sym2 := NewSymbol("66")
	fmt.Printf("%s %s %v", sym1, sym2, ValueEquals(sym1, sym2))
	// Output: 66 66 false
}
...
func TestPromiseExport(t *testing.T) {
	...
	p, _, _ := vm.NewPromise()
	pv := vm.ToValue(p)
	if actual := ValueExportType(pv); actual != reflect.TypeOf((*Promise)(nil)) {
		t.Fatalf("Export type: %v", actual)
	}

	if ev := ValueExport(pv); ev != p {
...
func ExampleRuntime_ForOf() {
	...
	ex := r.Try(func() {
		r.ForOf(v, func(v Value) bool {
			...
			sb.WriteString(ValueToString(key))
			sb.WriteString("=")
