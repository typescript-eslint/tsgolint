...
func (r *Runtime) stringproto_iterator(call FunctionCall) Value {
	...
	return r.createStringIterator(ValueToJsString(call.This))
}
...
func (r *Runtime) string_raw(call FunctionCall) Value {
	cooked := ValueToObject(r, call.Argument(0))
	raw := ValueToObject(r, nilSafe(cooked.self.getStr("raw", nil)))
	...
	for {
		nextSeg := ValueToJsString(nilSafe(raw.self.getIdx(valueInt(nextIndex), nil)))
		stringElements.WriteString(nextSeg)
		if nextIndex+1 == literalSegments {
			return stringElements.String()
		}
		if nextIndex < numberOfSubstitutions {
			stringElements.WriteString(ValueToJsString(nilSafe(call.Arguments[nextIndex+1])))
		}
		nextIndex++
	}
}
...
func (r *Runtime) stringproto_at(call FunctionCall) Value {
	r.checkObjectCoercible(call.This)
	s := ValueToJsString(call.This)
	pos := ValueToInteger(call.Argument(0))
...
func (r *Runtime) stringproto_codePointAt(call FunctionCall) Value {
	r.checkObjectCoercible(call.This)
	s := ValueToJsString(call.This)
	p := ValueToInteger(call.Argument(0))
	size := s.Length()
	if p < 0 || p >= int64(size) {
		return _undefined
	}
...
func (r *Runtime) stringproto_concat(call FunctionCall) Value {
	...
	a, u := devirtualizeString(ValueToJsString(call.This))
	allAscii := true
	totalLen := 0
	if u == nil {
		strs[0] = a
	...
	for i, arg := range call.Arguments {
		a, u := devirtualizeString(ValueToJsString(arg))
		if u != nil {
			allAscii = false
			totalLen += u.Length()
			strs[i+1] = u
	...
	if allAscii {
		...
		buf.Grow(totalLen)
		for _, s := range strs {
			buf.WriteString(ValueToString(s))
		}
...
func (r *Runtime) stringproto_endsWith(call FunctionCall) Value {
	...
	searchStr := ValueToJsString(searchString)
	l := int64(s.Length())
	var pos int64
	if posArg := call.Argument(1); posArg != _undefined {
		pos = ValueToInteger(posArg)
	} else {
		pos = l
	}
...
func (r *Runtime) stringproto_indexOf(call FunctionCall) Value {
	r.checkObjectCoercible(call.This)
	value := ValueToJsString(call.This)
	target := ValueToJsString(call.Argument(0))
	pos := ValueToInteger(ValueToNumber(call.Argument(1)))
...
func (r *Runtime) stringproto_localeCompare(call FunctionCall) Value {
	r.checkObjectCoercible(call.This)
	this := norm.NFD.String(ValueToString(ValueToJsString(call.This)))
	that := norm.NFD.String(ValueToString(ValueToJsString(call.Argument(0))))
...
func (r *Runtime) stringproto_match(call FunctionCall) Value {
	...
	if matcher, ok := r.toObject(rx.getSym(SymMatch, nil)).self.assertCallable(); ok {
		return matcher(FunctionCall{
			This:      rx.val,
			Arguments: []Value{ValueToJsString(call.This)},
		})
	}
	...
	if regexp != _undefined && regexp != _null {
		if isRegexp(regexp) {
			if o, ok := regexp.(*Object); ok {
				...
				if !strings.Contains(ValueToString(ValueToJsString(flags)), "g") {
...
func (r *Runtime) stringproto_normalize(call FunctionCall) Value {
	...
	if formArg := call.Argument(0); formArg != _undefined {
		form = ValueToString(ValueToJsString(ValueToJsString(formArg)))
	} else {
		form = "NFC"
	}
	var f norm.Form
...
func (r *Runtime) _stringPad(call FunctionCall, start bool) Value {
	...
	if fillString := call.Argument(1); fillString != _undefined {
		filler = ValueToJsString(fillString)
		if filler.Length() == 0 {
			return s
		}
...
func (r *Runtime) stringproto_repeat(call FunctionCall) Value {
	r.checkObjectCoercible(call.This)
	s := ValueToJsString(call.This)
	n := ValueToNumber(call.Argument(0))
	if n == _positiveInf {
		panic(r.newError(r.getRangeError(), "Invalid count value"))
	}
	numInt := ValueToInteger(n)
	if numInt < 0 {
		panic(r.newError(r.getRangeError(), "Invalid count value"))
	}
...
func getReplaceValue(replaceValue Value) (str String, rcall func(FunctionCall) Value) {
	...
	str = ValueToJsString(replaceValue)
	return
}
...
func stringReplace(s String, found [][]int, newstring String, rcall func(FunctionCall) Value) Value {
	...
	if rcall != nil {
		for _, item := range found {
			...
			replacement := ValueToJsString(rcall(FunctionCall{
				This:      _undefined,
...
func (r *Runtime) stringproto_replace(call FunctionCall) Value {
	...
	s := ValueToJsString(call.This)
	var found [][]int
	searchStr := ValueToJsString(searchValue)
...
func (r *Runtime) stringproto_slice(call FunctionCall) Value {
	...
	l := int64(s.Length())
	start := ValueToInteger(call.Argument(0))
	var end int64
	if arg1 := call.Argument(1); arg1 != _undefined {
		end = ValueToInteger(arg1)
	} else {
		end = l
	}
...
func (r *Runtime) stringproto_split(call FunctionCall) Value {
	...
	separatorValue = ValueToStringValue(separatorValue)
	...
	separator := ValueToString(separatorValue)
...
func (r *Runtime) stringproto_substring(call FunctionCall) Value {
	...
	l := int64(s.Length())
	intStart := ValueToInteger(call.Argument(0))
	var intEnd int64
	if end := call.Argument(1); end != _undefined {
		intEnd = ValueToInteger(end)
	} else {
		intEnd = l
	}
	if intStart < 0 {
...
func (r *Runtime) stringproto_trim(call FunctionCall) Value {
	...
	// TODO handle invalid UTF-16
	return newStringValue(strings.Trim(ValueToString(s), parser.WhitespaceChars))
}
...
func (r *Runtime) stringproto_trimEnd(call FunctionCall) Value {
	...
	// TODO handle invalid UTF-16
	return newStringValue(strings.TrimRight(ValueToString(s), parser.WhitespaceChars))
}
...
func (r *Runtime) stringproto_trimStart(call FunctionCall) Value {
	...
	// TODO handle invalid UTF-16
	return newStringValue(strings.TrimLeft(ValueToString(s), parser.WhitespaceChars))
}
