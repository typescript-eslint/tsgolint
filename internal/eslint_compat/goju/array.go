...
func (ai *arrayIterObject) next() Value {
	if ai.obj == nil {
		return ai.val.runtime.createLightIterResultObject(_undefined, true)
	}
	...
	if ai.kind == iterationKindKey {
		return ai.val.runtime.createLightIterResultObject(idxVal, false)
	}
	...
	return ai.val.runtime.createLightIterResultObject(result, false)
}
...
func (a *arrayObject) getOwnPropIdx(idx valueInt) Value {
	...
	return a.baseObject.getOwnPropStr(ValueToUnistring(idx))
}
...
func (a *arrayObject) setOwnIdx(idx valueInt, val Value, throw bool) bool {
	if i := toIdx(idx); i != math.MaxUint32 {
		...
		return a.baseObject.setOwnStr(ValueToUnistring(idx), val, throw)
	}
}
...
func (a *arrayObject) hasOwnPropertyIdx(idx valueInt) bool {
	...
	return a.baseObject.hasOwnPropertyStr(ValueToUnistring(idx))
}
...
func (r *Runtime) defineArrayLength(prop *valueProperty, descr PropertyDescriptor, setter func(uint32, bool) bool, throw bool) bool {
	...
	if descr.Value != nil {
		oldLen := uint32(ValueToInteger(prop.value))
		if oldLen != newLen {
...
func (a *arrayObject) defineOwnPropertyIdx(idx valueInt, descr PropertyDescriptor, throw bool) bool {
	...
	return a.baseObject.defineOwnPropertyStr(ValueToUnistring(idx), descr, throw)
}
...
func (a *arrayObject) _deleteIdxProp(idx uint32, throw bool) bool {
	if idx < uint32(len(a.values)) {
		if v := a.values[idx]; v != nil {
			if p, ok := v.(*valueProperty); ok {
				if !p.configurable {
					a.val.runtime.typeErrorResult(throw, "Cannot delete property '%d' of %s", idx, ValueToJsString(a.val))
					return false
				}
				a.propValueCount--
			}
...
func (a *arrayObject) deleteIdx(idx valueInt, throw bool) bool {
	...
	return a.baseObject.deleteStr(ValueToUnistring(idx), throw)
}
