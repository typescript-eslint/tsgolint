...
func (r *Runtime) builtin_isNaN(call FunctionCall) Value {
	if math.IsNaN(ValueToFloat(call.Argument(0))) {
		return valueTrue
	} else {
		return valueFalse
	}
...
func (r *Runtime) builtin_parseInt(call FunctionCall) Value {
	str := ValueToJsString(call.Argument(0)).toTrimmedUTF8()
...
func (r *Runtime) builtin_parseFloat(call FunctionCall) Value {
	m := parseFloatRegexp.FindStringSubmatch(ValueToJsString(call.Argument(0)).toTrimmedUTF8())
	if len(m) == 2 {
		if s := m[1]; s != "" && s != "+" && s != "-" {
			switch s {
			case "+", "-":
...
func (r *Runtime) builtin_decodeURI(call FunctionCall) Value {
	uriString := ValueToJsString(call.Argument(0))
