...
func (o *primitiveValueObject) export(*objectExportCtx) interface{} {
	return ValueExport(o.pValue)
}

func (o *primitiveValueObject) exportType() reflect.Type {
	return ValueExportType(o.pValue)
}
...
func (o *baseObject) hasPropertyIdx(idx valueInt) bool {
	return o.val.self.hasPropertyStr(ValueToUnistring(idx))
}
...
func (o *baseObject) getIdx(idx valueInt, receiver Value) Value {
	return o.val.self.getStr(ValueToUnistring(idx), receiver)
}
...
func (o *baseObject) getOwnPropIdx(idx valueInt) Value {
	return o.val.self.getOwnPropStr(ValueToUnistring(idx))
}
...
func (o *baseObject) deleteIdx(idx valueInt, throw bool) bool {
	return o.val.self.deleteStr(ValueToUnistring(idx), throw)
}

func (o *baseObject) deleteSym(s *Symbol, throw bool) bool {
	if o.symValues != nil {
		if val := o.symValues.get(s); val != nil {
			if !o.checkDelete(ValueToUnistring(s.descriptiveString()), val, throw) {
				return false
			}
...
func (o *baseObject) setOwnIdx(idx valueInt, val Value, throw bool) bool {
	return o.val.self.setOwnStr(ValueToUnistring(idx), val, throw)
}
...
func (o *baseObject) setForeignIdx(name valueInt, val, receiver Value, throw bool) (bool, bool) {
	...
	return o.setForeignStr(ValueToUnistring(name), val, receiver, throw)
}
...
func (o *baseObject) hasOwnPropertyIdx(idx valueInt) bool {
	return o.val.self.hasOwnPropertyStr(ValueToUnistring(idx))
}
		...
		if existing.accessor && descr.Value != nil || !existing.accessor && (getterObj != nil || setterObj != nil) {
			...
			if !existing.configurable {
				if !existing.writable {
					...
					if descr.Value != nil && !ValueSameAs(descr.Value, existing.value) {
						goto Reject
					}
				}
			}
...
func (o *baseObject) defineOwnPropertyIdx(idx valueInt, desc PropertyDescriptor, throw bool) bool {
	return o.val.self.defineOwnPropertyStr(ValueToUnistring(idx), desc, throw)
}
...
func (o *baseObject) defineOwnPropertySym(s *Symbol, descr PropertyDescriptor, throw bool) bool {
	...
	if v, ok := o._defineOwnProperty(ValueToUnistring(s.descriptiveString()), existingVal, descr, throw); ok {
		if o.symValues == nil {
			o.symValues = newOrderedMap(nil)
		}
		o.symValues.set(s, v)
...
func (o *baseObject) vmCall(vm *vm, _ int) {
	panic(vm.r.NewTypeError("Not a function: %s", ValueToJsString(o.val)))
}
...
func (o *baseObject) export(ctx *objectExportCtx) interface{} {
	...
	for _, itemName := range keys {
		itemNameStr := ValueToString(itemName)
		v := o.val.self.getStr(ValueToUnistring(itemName), nil)
		if v != nil {
...
func genericExportToMap(o *Object, dst reflect.Value, typ reflect.Type, ctx *objectExportCtx) error {
	...
	for item, next := iter.next(); next != nil; item, next = next() {
		...
		if needConvertKeys {
			...
			if err != nil {
				return fmt.Errorf("could not convert map key %s to %v: %w", ValueToString(item.name), typ, err)
			}
		} else {
			kv = reflect.ValueOf(ValueToString(item.name))
		}

		ival := o.self.getStr(ValueToUnistring(item.name), nil)
		if ival != nil {
			vv := reflect.New(elemTyp).Elem()
			err = r.toReflectValue(ival, vv, ctx)
			if err != nil {
				return fmt.Errorf("could not convert map value %v to %v at key %s: %w", ival, typ, ValueToString(item.name), err)
			}
...
func (i *recursivePropIter) next() (propIterItem, iterNextFunc) {
	for {
		...
		name := ValueToUnistring(item.name)
...
func (o *baseObject) hasInstance(Value) bool {
	panic(o.val.runtime.NewTypeError("Expecting a function in instanceof check, but got %s", ValueToJsString(o.val)))
}
...
func (o *Object) get(p Value, receiver Value) Value {
	...
	default:
		return o.self.getStr(ValueToUnistring(p), receiver)
	}
}
...
func (o *Object) getOwnProp(p Value) Value {
	...
	default:
		return o.self.getOwnPropStr(ValueToUnistring(p))
	}
}
...
func (o *Object) hasOwnProperty(p Value) bool {
	...
	default:
		return o.self.hasOwnPropertyStr(ValueToUnistring(p))
	}
}
...
func (o *Object) hasProperty(p Value) bool {
	...
	default:
		return o.self.hasPropertyStr(ValueToUnistring(p))
	}
}
...
func (o *Object) set(name Value, val, receiver Value, throw bool) bool {
	...
	default:
		return o.setStr(ValueToUnistring(name), val, receiver, throw)
	}
}
...
func (o *Object) setOwn(name Value, val Value, throw bool) bool {
	...
	default:
		return o.self.setOwnStr(ValueToUnistring(name), val, throw)
	}
}
...
func (o *Object) delete(n Value, throw bool) bool {
	...
	default:
		return o.self.deleteStr(ValueToUnistring(n), throw)
	}
}
...
func (o *Object) defineOwnProperty(n Value, desc PropertyDescriptor, throw bool) bool {
	...
	default:
		return o.self.defineOwnPropertyStr(ValueToUnistring(n), desc, throw)
	}
}
