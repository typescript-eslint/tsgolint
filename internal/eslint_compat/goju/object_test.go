...
func TestDefineProperty(t *testing.T) {
	...
	if v := o.Get("accessor_ro"); ValueToInteger(v) != 1 {
	...
	if v := o.Get("accessor_rw"); ValueToInteger(v) != 42 {
...
func TestExportWrappedMap(t *testing.T) {
	...
	exported := ValueExport(vm.ToValue(m))
...
func BenchmarkConv(b *testing.B) {
	count := int64(0)
	for i := 0; i < b.N; i++ {
		count += ValueToInteger(valueInt(123))
	}
	if count == 0 {
		b.Fatal("zero")
	}
