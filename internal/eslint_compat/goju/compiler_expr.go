	...
	if expr.constant() {
		if val, ex := c.evalConst(expr); ex == nil {
			if s, ok := val.(String); ok {
				return ValueToUnistring(s), true
			}
		}
	}
	return "", false
...
func (c *compiler) processKey(expr ast.Expression) (val unistring.String, computed bool) {
	...
	if keyExpr.constant() {
		...
		if ex == nil {
			return ValueToUnistring(v), false
		}
	}
...
func (c *compiler) emitThrow(v Value) {
	if o, ok := v.(*Object); ok {
		t := ValueToString(ValueToJsString(nilSafe(o.self.getStr("name", nil))))
		switch t {
	...
	c.assert(false, 0, "unknown exception type thrown while evaluating constant expression: %s", ValueToString(v))
	panic("unreachable")
}
