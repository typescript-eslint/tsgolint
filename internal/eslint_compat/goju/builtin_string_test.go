...
func TestValueStringBuilder(t *testing.T) {
	...
	t.Run("substringUnicode", func(t *testing.T) {
		...
		res := sb.String()
		if !ValueSameAs(res, unicodeStringFromRunes([]rune{0x10000})) {
			t.Fatal(res)
		}
	})
	...
	t.Run("substringASCIIUnicode", func(t *testing.T) {
		...
		res := sb.String()
		if !ValueSameAs(res, unicodeStringFromRunes([]rune{'a', 0xD800})) {
			t.Fatal(res)
		}
	})
	...
	t.Run("substringUnicodeASCII", func(t *testing.T) {
		...
		res := sb.String()
		if !ValueSameAs(res, unicodeStringFromRunes([]rune{0xDC00, 'b'})) {
			t.Fatal(res)
		}
	})
	...
	t.Run("concatSubstringUnicodeASCII", func(t *testing.T) {
		...
		if res := sb.String(); !ValueSameAs(res, newStringValue("юникод ascii")) {
			t.Fatal(res)
		}
	})
	...
	t.Run("concat_ASCII_importedUnicode", func(t *testing.T) {
		...
		s := sb.String()
		if res, ok := s.(unicodeString); !ok || !ValueSameAs(res, newStringValue("ascii imported_юникод")) {
			t.Fatal(s)
		}
	})
