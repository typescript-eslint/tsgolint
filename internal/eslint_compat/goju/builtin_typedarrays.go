...
func (ctx *typedArraySortCtx) Less(i, j int) bool {
	...
	if ctx.compare != nil {
		...
		res := ValueToNumber(ctx.compare(FunctionCall{
			This:      _undefined,
			Arguments: []Value{x, y},
		}))
...
func (r *Runtime) arrayBufferProto_slice(call FunctionCall) Value {
	...
	if b, ok := o.self.(*arrayBufferObject); ok {
		l := int64(len(b.data))
		start := relToIdx(ValueToInteger(call.Argument(0)), l)
		var stop int64
		if arg := call.Argument(1); arg != _undefined {
			stop = ValueToInteger(arg)
		} else {
			stop = l
		}
		...
		panic(r.NewTypeError("Species constructor did not return an ArrayBuffer: %s", ValueToString(ret)))
	}
	panic(r.NewTypeError("Object is not ArrayBuffer: %s", o))
}
...
func (r *Runtime) newDataView(args []Value, newTarget *Object) *Object {
	...
	if len(args) > 1 {
		...
		if byteOffset > len(buffer.data) {
			panic(r.newError(r.getRangeError(), "Start offset %s is outside the bounds of the buffer", ValueToString(offsetArg)))
		}
	}
...
func (r *Runtime) dataViewProto_getBigInt64(call FunctionCall) Value {
	if dv, ok := r.toObject(call.This).self.(*dataViewObject); ok {
		return (*valueBigInt)(dv.viewedArrayBuf.getBigInt64(dv.getIdxAndByteOrder(r.toIndex(ValueToNumber(call.Argument(0))), call.Argument(1), 8)))
	}
...
func (r *Runtime) dataViewProto_getBigUint64(call FunctionCall) Value {
	if dv, ok := r.toObject(call.This).self.(*dataViewObject); ok {
		return (*valueBigInt)(dv.viewedArrayBuf.getBigUint64(dv.getIdxAndByteOrder(r.toIndex(ValueToNumber(call.Argument(0))), call.Argument(1), 8)))
	}
...
func (r *Runtime) dataViewProto_setFloat64(call FunctionCall) Value {
	if dv, ok := r.toObject(call.This).self.(*dataViewObject); ok {
		idxVal := r.toIndex(call.Argument(0))
		val := ValueToFloat(call.Argument(1))
...
func (r *Runtime) typedArrayProto_copyWithin(call FunctionCall) Value {
	if ta, ok := r.toObject(call.This).self.(*typedArrayObject); ok {
		...
		l := int64(ta.length)
		var relEnd int64
		to := toIntStrict(relToIdx(ValueToInteger(call.Argument(0)), l))
		from := toIntStrict(relToIdx(ValueToInteger(call.Argument(1)), l))
...
func (r *Runtime) typedArrayProto_fill(call FunctionCall) Value {
	if ta, ok := r.toObject(call.This).self.(*typedArrayObject); ok {
		ta.viewedArrayBuf.ensureNotDetached(true)
		l := int64(ta.length)
		k := toIntStrict(relToIdx(ValueToInteger(call.Argument(1)), l))
		var relEnd int64
...
func (r *Runtime) typedArrayProto_set(call FunctionCall) Value {
	if ta, ok := r.toObject(call.This).self.(*typedArrayObject); ok {
		srcObj := ValueToObject(r, call.Argument(0))
		targetOffset := toIntStrict(ValueToInteger(call.Argument(1)))
		if targetOffset < 0 {
...
func (r *Runtime) typedArrayProto_slice(call FunctionCall) Value {
	if ta, ok := r.toObject(call.This).self.(*typedArrayObject); ok {
		ta.viewedArrayBuf.ensureNotDetached(true)
		length := int64(ta.length)
		start := toIntStrict(relToIdx(ValueToInteger(call.Argument(0)), length))
		var e int64
		if endArg := call.Argument(1); endArg != _undefined {
			e = ValueToInteger(endArg)
		} else {
			e = length
		}
...
func (r *Runtime) typedArrayProto_subarray(call FunctionCall) Value {
	if ta, ok := r.toObject(call.This).self.(*typedArrayObject); ok {
		l := int64(ta.length)
		beginIdx := relToIdx(ValueToInteger(call.Argument(0)), l)
		var relEnd int64
		if endArg := call.Argument(1); endArg != _undefined {
			relEnd = ValueToInteger(endArg)
		} else {
...
func (r *Runtime) typedArrayProto_with(call FunctionCall) Value {
	...
	default:
		numericValue = ValueToNumber(call.Argument(1))
	}
...
func checkTypedArrayMixBigInt(src, dst *Object) {
	srcType := ValueToString(src.self.getStr("name", nil))
	if strings.HasPrefix(srcType, "Big") {
		if !strings.HasPrefix(ValueToString(dst.self.getStr("name", nil)), "Big") {
