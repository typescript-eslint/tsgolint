...
func (r *objRef) refname() unistring.String {
	return ValueToUnistring(r.getKey())
}
...
func toNumeric(value Value) Value {
	...
	case *Object:
		...
		return ValueToNumber(primValue)
	}
...
func getFuncName(stack []Value, sb int) unistring.String {
	if sb > 0 {
		if f, ok := stack[sb-1].(*Object); ok {
			...
			return ValueToUnistring(nilSafe(f.self.getStr("name", nil)))
		}
	}
	return ""
}
...
func (vm *vm) toCallee(v Value) *Object {
	...
	panic(vm.r.NewTypeError("Value is not an object: %s", ValueToJsString(v)))
}
...
func (_add) exec(vm *vm) {
	...
	if isLeftString || isRightString {
		if !isLeftString {
			leftString = ValueToJsString(left)
		}
		if !isRightString {
			rightString = ValueToJsString(right)
		}
		...
		case valueInt:
			...
			default:
				ret = floatToValue(float64(left) + ValueToFloat(right))
			}
		case *valueBigInt:
		...
		default:
			...
			ret = floatToValue(ValueToFloat(left) + ValueToFloat(right))
		}
	}
...
func (_sub) exec(vm *vm) {
	...
	result = floatToValue(ValueToFloat(left) - ValueToFloat(right))
end:
	vm.sp--
...
func (_div) exec(vm *vm) {
	...
	left, right = ValueToFloat(leftValue), ValueToFloat(rightValue)
...
func (_mod) exec(vm *vm) {
	...
	result = floatToValue(math.Mod(ValueToFloat(left), ValueToFloat(right)))
end:
	vm.sp--
	vm.stack[vm.sp-1] = result
	vm.pc++
...
func (_plus) exec(vm *vm) {
	vm.stack[vm.sp-1] = ValueToNumber(vm.stack[vm.sp-1])
	vm.pc++
}
...
func (_toString) exec(vm *vm) {
	p := vm.sp - 1
	vm.stack[p] = ValueToJsString(vm.stack[p])
	vm.pc++
}
...
func (_getElemRef) exec(vm *vm) {
	obj := ValueToObject(vm.r, vm.stack[vm.sp-2])
...
func (_getElemRefRecv) exec(vm *vm) {
	obj := ValueToObject(vm.r, vm.stack[vm.sp-1])
...
func (_setElem) exec(vm *vm) {
	obj := ValueToObject(vm.r, vm.stack[vm.sp-3])
...
func (_setElem1Named) exec(vm *vm) {
	...
	base := ValueToObject(vm.r, receiver)
	propName := vm.stack[vm.sp-2]
	val := vm.stack[vm.sp-1]
...
func (p getPropRef) exec(vm *vm) {
	vm.refStack = append(vm.refStack, &objStrRef{
		base: ValueToObject(vm.r, vm.stack[vm.sp-1]),
...
func (p setProp) exec(vm *vm) {
	val := vm.stack[vm.sp-1]
	ValueToObject(vm.r, vm.stack[vm.sp-2]).self.setOwnStr(unistring.String(p), val, false)
...
func (g getProp) exec(vm *vm) {
	v := vm.stack[vm.sp-1]
	res, ok := ValueGoObjectGet(vm.r, v, string(g))
	if !ok {
		obj := ValueBaseObject(vm.r, v)
		if obj == nil {
...
func (g getPropRecv) exec(vm *vm) {
	...
	if !ok {
		...
		res = nilSafe(obj.self.getStr(unistring.String(g), recv))
	}
	vm.stack[vm.sp-2] = res
	vm.sp--
	vm.pc++
}
...
func (g getPropRecvCallee) exec(vm *vm) {
	...
	if !ok {
		...
		res = obj.self.getStr(n, recv)
		if res == nil {
			res = memberUnresolved{valueUnresolved{r: vm.r, ref: n}}
		}
	}
	vm.stack[vm.sp-1] = res
	vm.pc++
...
func (_getElem) exec(vm *vm) {
	v := vm.stack[vm.sp-2]
	propName := toPropertyKey(vm.stack[vm.sp-1])
	res, ok := ValueGoObjectGetAnyKey(vm.r, v, propName)
	if !ok {
		obj := ValueBaseObject(vm.r, v)
		if obj == nil {
		...
		res = nilSafe(obj.get(propName, v))
	}
	vm.stack[vm.sp-2] = res
...
func (_getElemRecv) exec(vm *vm) {
	...
	if !ok {
		...
		res = nilSafe(obj.get(propName, recv))
	}
	vm.stack[vm.sp-3] = res
...
func (_getKey) exec(vm *vm) {
	...
	if !ok {
		obj := ValueBaseObject(vm.r, v)
		if obj == nil {
			vm.throw(vm.r.NewTypeError("Cannot read property '%s' of undefined", ValueToString(propName)))
			return
		}
		res = nilSafe(obj.get(propName, v))
	}
...
func (_getElemCallee) exec(vm *vm) {
	...
	if !ok {
		...
		res = obj.get(propName, v)
		if res == nil {
			res = memberUnresolved{valueUnresolved{r: vm.r, ref: ValueToUnistring(propName)}}
		}
	}
	vm.stack[vm.sp-1] = res
...
func (_boxThis) exec(vm *vm) {
	...
	if v == _undefined || v == _null {
		...
		vm.stack[vm.sb] = ValueToObject(vm.r, v)
	}
	vm.pc++
}
...
func (numargs call) exec(vm *vm) {
	...
	// arg<numargs-1>
	n := int(numargs)
	// if vm.sp == 86 && vm.pc == 5 && vm.sb == 79  && vm.args == 2{
	// 	println("foo")
	// }
...
func (j jneP) exec(vm *vm) {
	vm.sp--
	if !ValueToBoolean(vm.stack[vm.sp]) {
		vm.pc += int(j)
	} else {
		vm.pc++
	}
...
func (j jeq) exec(vm *vm) {
	if ValueToBoolean(vm.stack[vm.sp-1]) {
		vm.pc += int(j)
	} else {
		vm.sp--
		vm.pc++
...
func cmp(px, py Value) Value {
	...
	if nx, ny := ValueToFloat(px), ValueToFloat(py); math.IsNaN(nx) || math.IsNaN(ny) {
		return _undefined
	} else {
		ret = nx < ny
	}
...
func (_op_eq) exec(vm *vm) {
	if ValueEquals(vm.stack[vm.sp-2], vm.stack[vm.sp-1]) {
		vm.stack[vm.sp-2] = valueTrue
	} else {
		vm.stack[vm.sp-2] = valueFalse
	}
...
func (_op_strict_eq) exec(vm *vm) {
	if ValueStrictEquals(vm.stack[vm.sp-2], vm.stack[vm.sp-1]) {
		vm.stack[vm.sp-2] = valueTrue
	} else {
		vm.stack[vm.sp-2] = valueFalse
	}
...
func (_enterWith) exec(vm *vm) {
	vm.newStash()
	vm.stash.obj = ValueToObject(vm.r, vm.stack[vm.sp-1])
	vm.sp--
	vm.pc++
}
...
func (_enumerate) exec(vm *vm) {
	...
	if v == _undefined || v == _null {
		...
		vm.iterStack = append(vm.iterStack, iterStackItem{f: enumerateRecursive(ValueToObject(vm.r, v))})
	}
	vm.sp--
	vm.pc++
}
...
func (r *Runtime) copyDataProperties(target, source Value) {
	...
	sourceObj := ValueToObject(r, source)
