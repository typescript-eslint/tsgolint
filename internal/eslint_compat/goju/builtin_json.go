...
func (r *Runtime) builtinJSON_parse(call FunctionCall) Value {
	d := json.NewDecoder(strings.NewReader(ValueToString(ValueToJsString(call.Argument(0)))))
	...
	if arg1 := call.Argument(1); arg1 != _undefined {
		reviver, _ = ValueToObject(r, arg1).self.assertCallable()
	}
	...
	if object, ok := value.(*Object); ok {
		if isArray(object) {
			...
			for _, name := range object.self.stringKeys(false, nil) {
				value := r.builtinJSON_reviveWalk(reviver, object, name)
				if value == _undefined {
					object.self.deleteStr(ValueToUnistring(name), false)
				} else {
					createDataProperty(object, name, value)
				}
			}
		...
		if o, ok := spaceValue.(*Object); ok {
			...
			case *primitiveValueObject:
				...
				case valueInt, valueFloat:
					spaceValue = ValueToNumber(o)
				}
			case *stringObject:
				spaceValue = ValueToStringValue(o)
			}
		}
		isNum := false
		var num int64
...
func (ctx *_builtinJSON_stringifyContext) str(key Value, holder *Object) bool {
	...
	if o, ok := value.(*Object); ok {
		...
		case *primitiveValueObject:
			...
			case valueInt, valueFloat:
				value = ValueToNumber(o)
			default:
				value = pValue
			}
		case *stringObject:
			value = ValueToJsString(o)
	...
	} else if v, ok := value.(json.Marshaler); ok {
		b, err := v.MarshalJSON()
		if err != nil {
			panic(ctx.r.NewGoError(err))
		}
	...
	case String:
		ctx.quote(value1)
	case valueInt:
		ctx.buf.WriteString(ValueToString(value))
	case valueFloat:
...
func (ctx *_builtinJSON_stringifyContext) jo(object *Object) {
	...
	for _, name := range props {
		...
		ctx.quote(ValueToJsString(name))
		if ctx.gap != "" {
			ctx.buf.WriteString(": ")
		} else {
			ctx.buf.WriteByte(':')
