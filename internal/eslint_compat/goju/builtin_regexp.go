		...
		if utf16.IsSurrogate(c) {
			if sb.Len() == 0 {
				...
				for p := 0; p < pos; {
					...
					if err != nil {
						// will not happen
						panic(fmt.Errorf("error while reading string head %q, pos: %d: %w", ValueToString(s), pos, err))
					}
					sb.WriteRune(c)
					p += size
				}
...
func (r *Runtime) NewRegExp(patternVal, flagsVal Value) *Object {
	return r.newRegExp(patternVal, flagsVal, r.getRegExpPrototype()).val
}
...
func (r *Runtime) newRegExp(patternVal, flagsVal Value, proto *Object) *regexpObject {
		...
		if obj, ok := patternVal.(*Object); ok {
			if rx, ok := obj.self.(*regexpObject); ok {
				if flagsVal == nil || flagsVal == _undefined {
					...
					return r._newRegExp(rx.source, ValueToString(ValueToJsString(flagsVal)), proto)
				}
			} else {
				pattern = ValueToJsString(nilSafe(obj.self.getStr("source", nil)))
				if flagsVal == nil || flagsVal == _undefined {
					flags = ValueToString(ValueToJsString(nilSafe(obj.self.getStr("flags", nil))))
				} else {
					flags = ValueToString(ValueToJsString(flagsVal))
				}
				goto exit
			}
		}
	...
	if patternVal != nil && patternVal != _undefined {
		pattern = ValueToJsString(patternVal)
	}
	if flagsVal != nil && flagsVal != _undefined {
		flags = ValueToString(ValueToJsString(flagsVal))
	}
...
func (r *Runtime) regexpproto_compile(call FunctionCall) Value {
	if this, ok := r.toObject(call.This).self.(*regexpObject); ok {
		...
		if patternVal != _undefined {
			source = ValueToJsString(patternVal)
		} else {
			source = stringEmpty
		}
		if flagsVal != _undefined {
...
func (r *Runtime) regexpproto_exec(call FunctionCall) Value {
	if this, ok := r.toObject(call.This).self.(*regexpObject); ok {
		return this.exec(ValueToJsString(call.Argument(0)))
	} else {
...
func (r *Runtime) regexpproto_test(call FunctionCall) Value {
	if this, ok := r.toObject(call.This).self.(*regexpObject); ok {
		if this.test(ValueToJsString(call.Argument(0))) {
			return valueTrue
		} else {
			return valueFalse
		}
...
func (r *Runtime) regexpproto_toString(call FunctionCall) Value {
	...
	pattern := ValueToJsString(nilSafe(obj.self.getStr("source", nil)))
	flags := ValueToJsString(nilSafe(obj.self.getStr("flags", nil)))
	var sb StringBuilder
	sb.WriteRune('/')
...
func (r *Runtime) getGlobalRegexpMatches(rxObj *Object, s String, fullUnicode bool) []Value {
	...
	for {
		...
		a = append(a, res)
		matchStr := ValueToJsString(nilSafe(r.toObject(res).self.getIdx(valueInt(0), nil)))
...
func (r *Runtime) regexpproto_stdMatcherGeneric(rxObj *Object, s String) Value {
	rx := rxObj.self
	flags := ValueToString(nilSafe(rx.getStr("flags", nil)))
	...
	if global {
		...
		for _, result := range a {
			...
			matchStr := ValueToStringValue(nilSafe(obj.self.getIdx(valueInt(0), nil)))
...
func (r *Runtime) regexpproto_stdMatcher(call FunctionCall) Value {
	thisObj := r.toObject(call.This)
	s := ValueToJsString(call.Argument(0))
...
func (r *Runtime) regexpproto_stdSearchGeneric(rxObj *Object, arg String) Value {
	...
	zero := intToValue(0)
	if !ValueSameAs(previousLastIndex, zero) {
	...
	if !ValueSameAs(currentLastIndex, previousLastIndex) {
...
func (r *Runtime) regexpproto_stdMatcherAll(call FunctionCall) Value {
	thisObj := r.toObject(call.This)
	s := ValueToJsString(call.Argument(0))
	flags := ValueToJsString(nilSafe(thisObj.self.getStr("flags", nil)))
...
func (ri *regExpStringIterObject) next() (v Value) {
	...
	matchStr := ValueToJsString(nilSafe(ri.val.runtime.toObject(match).self.getIdx(valueInt(0), nil)))
	if matchStr.Length() == 0 {
...
func (r *Runtime) regexpproto_stdSplitterGeneric(splitter *Object, s String, limit Value, unicodeMatching bool) Value {
	...
	size := s.Length()
	p := 0
	execFn := toMethod(ValueToObject(r, splitter).self.getStr("exec", nil)) // must be non-nil
...
func (r *Runtime) regexpproto_stdSplitter(call FunctionCall) Value {
	...
	if search == nil || c != r.global.RegExp {
		flags := ValueToJsString(nilSafe(rxObj.self.getStr("flags", nil)))
		flagsStr := ValueToString(flags)
...
func (r *Runtime) regexpproto_stdReplacerGeneric(rxObj *Object, s, replaceStr String, rcall func(FunctionCall) Value) Value {
	var results []Value
	flags := ValueToString(nilSafe(rxObj.self.getStr("flags", nil)))
	...
	for _, result := range results {
		...
		matched := ValueToJsString(nilSafe(obj.self.getIdx(valueInt(0), nil)))
		matchLength := matched.Length()
		position := toIntStrict(max(min(ValueToInteger(nilSafe(obj.self.getStr("index", nil))), int64(lengthS)), 0))
		var captures []Value
		if rcall != nil {
			...
			if position >= nextSourcePosition {
				...
				writeSubstitution(s, position, len(captures), func(idx int) String {
					...
					if capture != _undefined {
						return ValueToJsString(capture)
					}
					return stringEmpty
