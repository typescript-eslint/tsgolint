...
func testMapHashVal(v1, v2 Value, expected bool, t *testing.T) {
	var h maphash.Hash
	actual := ValueHash(v1, &h) == ValueHash(v2, &h)
	if actual != expected {
...
func TestOrderedMap(t *testing.T) {
	...
	for i := int64(0); i < 50; i++ {
		...
		if !ValueSameAs(expected, actual) {
...
func TestOrderedMapCollision(t *testing.T) {
	...
	if n2Val := m.get(n2Key); !ValueSameAs(asciiString("n2"), n2Val) {
		t.Fatalf("unexpected n2Val: %v", n2Val)
	}
	if n1Val := m.get(n1Key); !ValueSameAs(asciiString("n1"), n1Val) {
...
func TestOrderedMapIter(t *testing.T) {
	...
	if !ValueSameAs(intToValue(1), ent.key) {
		t.Fatal("unexpected key")
	}
	if !ValueSameAs(valueTrue, ent.value) {
