...
func (r *Runtime) object_getPrototypeOf(call FunctionCall) Value {
	o := ValueToObject(r, call.Argument(0))
	p := o.self.proto()
	if p == nil {
		return _null
	}
...
func (r *Runtime) object_getOwnPropertyNames(call FunctionCall) Value {
	obj := ValueToObject(r, call.Argument(0))
...
func (r *Runtime) toValueProp(v Value) *valueProperty {
	...
	ret := &valueProperty{}
	if writable != nil && ValueToBoolean(writable) {
		ret.writable = true
	}
	if e := obj.self.getStr("enumerable", nil); e != nil && ValueToBoolean(e) {
		ret.enumerable = true
	}
	if c := obj.self.getStr("configurable", nil); c != nil && ValueToBoolean(c) {
...
func (r *Runtime) toPropertyDescriptor(v Value) (ret PropertyDescriptor) {
	if o, ok := v.(*Object); ok {
		...
		if p := descr.getStr("writable", nil); p != nil {
			ret.Writable = ToFlag(ValueToBoolean(p))
		}
		if p := descr.getStr("enumerable", nil); p != nil {
			ret.Enumerable = ToFlag(ValueToBoolean(p))
		}
		if p := descr.getStr("configurable", nil); p != nil {
			ret.Configurable = ToFlag(ValueToBoolean(p))
		}
		...
		r.typeErrorResult(true, "Property description must be an object: %s", ValueToString(v))
	}
...
func (r *Runtime) object_create(call FunctionCall) Value {
	...
	if arg := call.Argument(0); arg != _null {
		if o, ok := arg.(*Object); ok {
			...
			r.typeErrorResult(true, "Object prototype may only be an Object or null: %s", ValueToString(arg))
		}
	}
...
func (r *Runtime) objectproto_getProto(call FunctionCall) Value {
	proto := ValueToObject(r, call.This).self.proto()
	if proto != nil {
		return proto
	}
	return _null
...
func (r *Runtime) objectproto_valueOf(call FunctionCall) Value {
	return ValueToObject(r, call.This)
}

func (r *Runtime) object_assign(call FunctionCall) Value {
	to := ValueToObject(r, call.Argument(0))
	if len(call.Arguments) > 1 {
		for _, arg := range call.Arguments[1:] {
			if arg != _undefined && arg != _null {
				source := ValueToObject(r, arg)
...
func (r *Runtime) object_is(call FunctionCall) Value {
	return r.toBoolean(ValueSameAs(call.Argument(0), call.Argument(1)))
}
