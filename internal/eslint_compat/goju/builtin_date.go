	...
	default: // one argument
		...
		if !valid {
			...
			if val, ok := pv.(String); ok {
				return dateParse(ValueToString(val))
			}
			pv = ValueToNumber(pv)
			var n int64
...
func (r *Runtime) date_parse(call FunctionCall) Value {
	t, set := dateParse(ValueToString(ValueToJsString(call.Argument(0))))
	if set {
...
func (r *Runtime) dateproto_toPrimitive(call FunctionCall) Value {
	...
	if ValueStrictEquals(asciiString("string"), arg) || ValueStrictEquals(asciiString("default"), arg) {
		return o.ordinaryToPrimitiveString()
	}
	if ValueStrictEquals(asciiString("number"), arg) {
...
func (r *Runtime) dateproto_setTime(call FunctionCall) Value {
	...
	if d, ok := obj.self.(*dateObject); ok {
		...
		return d.setTimeMs(ValueToInteger(n))
	}
...
func _intArg(call FunctionCall, argNum int) (int64, bool) {
	n := ValueToNumber(call.Argument(argNum))
	if IsNaN(n) {
		return 0, false
	}
	return ValueToInteger(n), true
}
