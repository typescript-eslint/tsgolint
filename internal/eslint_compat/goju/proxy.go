...
func (h *jsProxyHandler) getOwnPropertyDescriptorIdx(target *Object, prop valueInt) (Value, bool) {
	return h.proxyCall(proxy_trap_getOwnPropertyDescriptor, target, ValueToJsString(prop))
}
...
func (h *jsProxyHandler) definePropertyIdx(target *Object, prop valueInt, desc PropertyDescriptor) (bool, bool) {
	return h.boolProxyCall(proxy_trap_defineProperty, target, ValueToJsString(prop), desc.toValue(h.handler.runtime))
}
...
func (h *jsProxyHandler) hasIdx(target *Object, prop valueInt) (bool, bool) {
	return h.boolProxyCall(proxy_trap_has, target, ValueToJsString(prop))
}
...
func (h *jsProxyHandler) getIdx(target *Object, prop valueInt, receiver Value) (Value, bool) {
	return h.proxyCall(proxy_trap_get, target, ValueToJsString(prop), receiver)
}
...
func (h *jsProxyHandler) setIdx(target *Object, prop valueInt, value Value, receiver Value) (bool, bool) {
	return h.boolProxyCall(proxy_trap_set, target, ValueToJsString(prop), value, receiver)
}
...
func (h *jsProxyHandler) deleteIdx(target *Object, prop valueInt) (bool, bool) {
	return h.boolProxyCall(proxy_trap_deleteProperty, target, ValueToJsString(prop))
}
...
func (p *proxyObject) proxyGetChecks(targetProp, trapResult Value, name fmt.Stringer) {
	if targetDesc, ok := targetProp.(*valueProperty); ok {
		if !targetDesc.accessor {
			if !targetDesc.writable && !targetDesc.configurable && !ValueSameAs(trapResult, targetDesc.value) {
	...
	if v, ok := p.checkHandler().ownKeys(target); ok {
		...
		for k := int64(0); k < l; k++ {
			...
			if _, ok := item.(String); !ok {
				if _, ok := item.(*Symbol); !ok {
					panic(p.val.runtime.NewTypeError("%s is not a valid property name", ValueToString(item)))
				}
			}
		...
		for item, next := target.self.iterateKeys()(); next != nil; item, next = next() {
			if _, exists := keySet[item.name]; exists {
				...
				if !ext {
					panic(p.val.runtime.NewTypeError("'ownKeys' on proxy: trap result did not include '%s'", ValueToString(item.name)))
				}
				var prop Value
				if item.value == nil {
				...
				if prop, ok := prop.(*valueProperty); ok && !prop.configurable {
					panic(p.val.runtime.NewTypeError("'ownKeys' on proxy: trap result did not include non-configurable '%s'", ValueToString(item.name)))
				}
			}
		}
	...
	if !current.configurable {
		...
		if desc.IsData() && !current.accessor {
			if !current.configurable {
				...
				if !current.writable {
					if desc.Value != nil && !ValueSameAs(desc.Value, current.value) {
						return false
					}
				}
			}
		...
		if desc.IsAccessor() && current.accessor {
			if !current.configurable {
				if desc.Setter != nil && ValueSameAs(desc.Setter, current.setterFunc) {
					return false
				}
				if desc.Getter != nil && ValueSameAs(desc.Getter, current.getterFunc) {
					return false
				}
			}
		}
		...
		for i, val := range vals {
			...
			if symbols {
				...
				if _, ok := val.(*Symbol); !ok {
					prop = p.getOwnPropStr(ValueToUnistring(val))
				} else {
					continue
				}
			}
